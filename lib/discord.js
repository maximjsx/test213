import { COURSE_GUILD_IDS } from './specialTopics'

const API = 'https://discord.com/api'
const TIMEOUT_MS = 4000
const SYNC_INTERVAL_MS = 6 * 3600000
// "I joined, check again" may force a sync, but not more often than this
const FORCED_SYNC_MIN_MS = 30000
...
export async function fetchCourseGuildIds(accessToken) {
  if (!COURSE_GUILD_IDS.length) return []
  const guilds = await fetch(`${API}/users/@me/guilds`, {
    headers: { Authorization: `Bearer ${accessToken}` },
    signal: AbortSignal.timeout(TIMEOUT_MS),
  }).then(r => r.json())
  if (!Array.isArray(guilds)) return []
  return guilds.map(g => g.id).filter(id => COURSE_GUILD_IDS.includes(id))
}
...
// old CDN URL then 404s; server memberships change too. Re-read both every few
// hours so they stay live. Returns the updated profile, or null when nothing
// was refreshed.
export async function syncDiscordProfile(users, discordId, { force = false } = {}) {
  const staleBefore = new Date(Date.now() - (force ? FORCED_SYNC_MIN_MS : SYNC_INTERVAL_MS))
  ...
  try {
    ...
    const profile = {
      ...
      avatar: me.avatar,
      guildIds: await fetchCourseGuildIds(auth.accessToken),
    }
