...
// Community recordings, keyed by spoken text. Loaded once per page; a
// recorded phrase plays instead of TTS in every exercise that says it.
let _voiceMap = null
let _voiceMapPromise = null

function loadVoiceMap() {
  if (!_voiceMapPromise) {
    _voiceMapPromise = fetch('/api/voiceovers', { signal: AbortSignal.timeout(4000) })
      .then(r => (r.ok ? r.json() : {}))
      .catch(() => ({}))
      .then(d => (_voiceMap = d.map || {}))
  }
  return _voiceMapPromise
}
if (typeof window !== 'undefined') loadVoiceMap()

async function recordedUrl(text) {
  if (!text) return null
  const map = _voiceMap || await loadVoiceMap()
  return map[voiceKey(text)] || null
}

export async function speakBulgarian(text, voice) {
  if (typeof window === 'undefined') return 0
  if (_ttsMuted) return 0
  const recorded = await recordedUrl(text)
  if (recorded) return playAudioUrl(recorded)
...
export async function playClipBlocking({ audio, text, voice } = {}) {
  if (audio && audio.url) return playUrlUntilEnded(audio.url)
  const recorded = await recordedUrl(text)
  if (recorded) return playUrlUntilEnded(recorded)
