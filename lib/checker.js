...
export function checkAnswer(userInput, correctAnswer, options = {}) {
  ...
  // Support both single answer and array of accepted answers
  const answers = Array.isArray(correctAnswer) ? correctAnswer : [correctAnswer]
  const corrects = answers.map(normalize)
  ...
  // Typo tolerance: 1 per 5 chars, min 1 — check against all accepted answers
  const maxDist = Math.max(1, Math.floor(primary.length / 5))
  const minDist = Math.min(...corrects.map(c => levenshtein(user, c)))
  if (minDist <= maxDist) {
