...
const QUEST_POOL = [
  { id: 'earn_coins_40',   icon: '/icons/lightning.png', title: 'Earn 40 coins',                    goal: 40, reward: { type: 'coins', amount: 15 } },
  { id: 'earn_coins_60',   icon: '/icons/lightning.png', title: 'Earn 60 coins',                    goal: 60, reward: { type: 'coins', amount: 25 } },
  { id: 'lessons_2',    icon: '/icons/green_checkmark.png', title: 'Finish 2 lessons',          goal: 2,  reward: { type: 'coins', amount: 15 } },
  { id: 'lessons_3',    icon: '/icons/green_checkmark.png', title: 'Finish 3 lessons',          goal: 3,  reward: { type: 'coins', amount: 20 } },
  { id: 'combo_5',      icon: '/icons/fire.png',      title: 'Get 5 in a row in one lesson',    goal: 1,  reward: { type: 'coins', amount: 15 } },
  { id: 'combo_8',      icon: '/icons/fire.png',      title: 'Get 8 in a row in one lesson',    goal: 1,  reward: { type: 'coins', amount: 20 } },
  { id: 'accuracy_90',  icon: '/icons/star.png',      title: 'Score 90%+ in a lesson',          goal: 1,  reward: { type: 'coins', amount: 15 } },
...
export function ensureQuests(state, today) {
  if (state.quests?.day === today) return state
  return { ...state, quests: generateDailyQuests(today) }
}

// Called after every finished session (lesson, practice, game or review)
export function applySessionToQuests(quests, { coinsEarned = 0, accuracyPct = 0, maxCombo = 0, perfect = false, isLesson = true }) {
  if (!quests?.items) return quests
  const items = quests.items.map(q => {
    if (q.claimed) return q
    let inc = 0
    if (q.id.startsWith('earn_coins')) inc = coinsEarned
