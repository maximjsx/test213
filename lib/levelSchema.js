...
import { REGISTERS, MAX_USAGE_LENGTH } from './registers.js'
...
export function validateExercise(ex) {
  ...
  const problems = []
  rule(ex, problems)
  if (ex.register !== undefined) needs(problems, ex.register in REGISTERS, `Unknown register "${ex.register}".`)
  if (ex.usage !== undefined) needs(problems, String(ex.usage).length <= MAX_USAGE_LENGTH, `Usage note is over ${MAX_USAGE_LENGTH} characters.`)
  return problems
}

const DISCORD_ID = /^\d{17,20}$/
const DISCORD_INVITE = /^https:\/\/(discord\.gg|discord\.com\/invite)\/[\w-]+$/

// A special topic is locked behind coins, a Discord server, or both
function specialProblems(special) {
  if (!special) return []
  const p = []
  const { price, guild } = special
  if (price !== undefined) needs(p, Number.isInteger(price) && price > 0, 'Price must be a whole number above 0.')
  if (guild) {
    needs(p, DISCORD_ID.test(guild.id || ''), 'Discord server id should be 17 to 20 digits.')
    needs(p, filled(guild.name), 'Give the Discord server a name.')
    if (filled(guild.invite)) needs(p, DISCORD_INVITE.test(guild.invite), 'Invite should look like https://discord.gg/abc123.')
  }
  needs(p, price !== undefined || guild, 'A special topic needs a price, a Discord server, or both.')
  return p
}
...
export function validateLevel(level) {
  ...
  add(result.level, undefined, [
    ...(!filled(level.title) ? ['Topic has no title.'] : []),
    ...(!level.lessons?.length ? ['Topic has no lessons.'] : []),
    ...specialProblems(level.special),
  ])
  ...
  for (const [li, lesson] of (level.lessons || []).entries()) {
    ...
    needs(lessonProblems, Number.isInteger(lesson.coins) && lesson.coins > 0, 'Coin reward must be a whole number above 0.')
