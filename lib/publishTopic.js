...
function lessonIdProblems(level, otherLevels) {
  ...
  if (ids.size !== level.lessons.length) return ['Two lessons in this topic share an id.']
...
export function topicProblems(level, otherLevels) {
  if (!level || typeof level !== 'object') return ['That is not a topic.']
  if (!level.id || !ID_PATTERN.test(level.id)) return ['The topic needs an id made of letters, digits, _ or -.']
  if (!Array.isArray(level.lessons)) return ['The topic has no lessons.']
